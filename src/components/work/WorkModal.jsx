...
const WorkModal = ({ project, onClose }) => {
  ...
  // reset loading state setiap kali index/gambar berganti
  useEffect(() => {
    setImgLoaded(false);
  ...
  return createPortal(
    <div className="workitems__modal" onClick={onClose}>
      ...
      >
        ...
        <div className="workitems__modal-right">
          {!imgLoaded && <div className="workitems__modal-img-skeleton" />}

          <img
            loading="lazy"
            src={image[currentIndex]}
            alt={`${project.title} ${currentIndex + 1}`}
            className={`workitems__modal-img ${imgLoaded ? "is-loaded" : ""}`}
